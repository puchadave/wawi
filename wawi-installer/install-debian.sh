...
if [[ "$DEPLOY_MODE" == "docker" ]]; then
    ...
    ensure_stack_root "$INSTALL_DIR" || warn "ensure_stack_root fehlgeschlagen (docker)"
    ensure_stack_root "$INSTALL_DIR" || warn "ensure_stack_root fehlgeschlagen (docker)"
...
ENVEOF
    ...
    if ! require_command psql && ! require_command pg_isready; then
        warn "Postgres nicht gefunden - installiere (standard)..."
        install_postgres_debian || true
    fi
    if ! require_command psql && ! require_command pg_isready; then
        fatal "Postgres installation failed"
    fi
    if ! require_command redis-cli && ! require_command redis-server; then
        warn "Redis nicht gefunden - installiere (standard)..."
        install_redis_debian || true
    fi
    if ! require_command redis-cli && ! require_command redis-server; then
    ...
    if ! require_command node; then
        info "Installiere Node.js via install_node_debian..."
...
if [[ -f "$INSTALL_DIR/stack-root/pucha.dev" ]]; then
    ...
    warn "stack-root/pucha.dev nicht gefunden - pruefe $INSTALL_DIR/stack-root/"
fi
if [[ "$DEPLOY_MODE" == "docker" ]]; then
